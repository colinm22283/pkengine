...
namespace PKEngine {
    ...
    public:
        ...
        consteval Version(unsigned int _major, unsigned int _minor, unsigned int _patch) noexcept:
          major(_major), minor(_minor), patch(_patch) { }

        consteval Version(float version, unsigned int _patch) noexcept:
            major((unsigned int) version), minor(get_minor_version(version)), patch(_patch) { }
        consteval Version(float version):
            Version(version, 0) { }
